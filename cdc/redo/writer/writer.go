...
func NewRedoLogWriter(
	...
	scheme := uri.Scheme
	if !common.IsValidConsistentStorage(scheme) {
		return nil, cerror.ErrConsistentStorage.GenWithStackByArgs(scheme)
	}
	if common.ConsistentStorage(scheme) == common.ConsistentStorageBlackhole {
	...
	lwCfg := &logWriterConfig{
		...
		URI:                *uri,
		UseExternalStorage: common.IsExternalStorage(scheme),
...
func newLogWriter(
	...
	if cfg.UseExternalStorage {
		lw.extStorage, err = common.InitExternalStorage(ctx, cfg.URI)
		if err != nil {
			return nil, err
		}
...
func (l *logWriter) writeDeletedMarkerToS3(ctx context.Context) error {
	return cerror.WrapError(cerror.ErrS3StorageAPI, l.extStorage.WriteFile(ctx, l.getDeletedChangefeedMarker(), []byte("D")))
}
...
func (l *logWriter) getMetafileName() string {
	if model.DefaultNamespace == l.cfg.ChangeFeedID.Namespace {
		return fmt.Sprintf("%s_%s_%s%s", l.cfg.CaptureID, l.cfg.ChangeFeedID.ID,
			common.RedoMetaFileType, common.MetaEXT)
	}
...
func (l *logWriter) writeMetaToS3(ctx context.Context) error {
	...
	return cerror.WrapError(cerror.ErrS3StorageAPI, l.extStorage.WriteFile(ctx, l.getMetafileName(), fileData))
}
