...
func TestConsistentConfig(t *testing.T) {
	...
	for _, lc := range levelCases {
		require.Equal(t, lc.valid, common.IsValidConsistentLevel(lc.level))
	}
	...
	for _, lc := range levelEnableCases {
		require.Equal(t, lc.consistent, common.IsConsistentEnabled(lc.level))
	}
	...
	for _, sc := range storageCases {
		require.Equal(t, sc.valid, common.IsValidConsistentStorage(sc.storage))
	}
	...
	for _, sc := range s3StorageCases {
		require.Equal(t, sc.s3Enabled, common.IsExternalStorage(sc.storage))
	}
}
